...
__global__ void dequantizeThread8b(ElementT* dst,
                                   ...
                                   int rows,
                                   int columns,
                                   int thrd_row_blks) {  // Number of thread blocks along row dimension
  ...
  const auto total_row_blks = (rows + QuantBlk::kRow - 1) / QuantBlk::kRow;           // Total blocks along row dim for scales/zp
  const auto total_col_blks = (columns + QuantBlk::kColumn - 1) / QuantBlk::kColumn;  // Total blocks along col dim for scales/zp

  // Total number of blocks to process = total_row_blks * total_col_blks
  ...
  // Let's adjust based on the original caller's `total_thrd_blks` logic.
  ...
  const auto r_blk_idx_thread = static_cast<int32_t>(block_idx % thrd_row_blks);  // Thread block index along rows
  const auto c_blk_idx_thread = static_cast<int32_t>(block_idx / thrd_row_blks);  // Thread block index along columns
  ...
  for (int32_t c = c_start; c < c_end; ++c) {
    ...
    for (int32_t r = r_start; r < r_end; ++r) {
      ...
      const uint8_t zp_uint8 = (zero_points == nullptr) ? 128 : zero_points[scale_zp_flat_idx];  // Default ZP = 128

      // Get the quantized value (uint8_t)
      // Assuming weights are stored col-major for block quantization (e.g. [rows/block_size, cols] * block_size ?)
      // Let's assume standard row-major logical layout for weights access before block mapping: index = c * rows + r
      // And that weights pointer is already correctly base-offset.
      // Need clarification on `weights` layout if it's block-transposed.
      // Assuming simple logical layout access for now:
      const size_t q_val_idx = static_cast<size_t>(c) * rows + r;  // !! Assumes standard layout access, might be wrong for blockwise format
...
static void dequantize8b_generic(ElementT* dst, const uint8_t* weights, const ElementT* scales,
  ...
  // Calculate number of thread blocks needed based on total elements and ThreadBlk size
  // This matches the original calculation approach
  ...
  const auto total_thrd_blks = thrd_row_blks * thrd_col_blks;  // Total work items
  ...
  dequantizeThread8b<ElementT, block_size, qbits, Columnwise><<<grids, GridDim::maxThreadsPerBlock, 0, stream>>>(
      ...
      rows,
      columns,
      thrd_row_blks);  // Pass thrd_row_blks for 2D index reconstruction inside kernel
}
