...
using Forward = ForwardToFactory<ProviderBridgeEpFactory>;

ProviderBridgeEpFactory::ProviderBridgeEpFactory(const std::string& ep_name, const std::string& vendor,
...
bool ProviderBridgeEpFactory::GetDeviceInfoIfSupported(const OrtHardwareDevice* device,
...
OrtStatus* ProviderBridgeEpFactory::CreateEp(const OrtHardwareDevice* const* /*devices*/,
                                             const OrtKeyValuePairs* const* /*ep_metadata_pairs*/,
                                             size_t /*num_devices*/,
                                             const OrtSessionOptions* /*api_session_options*/,
                                             const OrtLogger* /*api_logger*/,
                                             OrtEpApi::OrtEp** /*ep*/) {
  ORT_THROW("CreateIExecutionProvider should be used for ProviderBridgeEpFactory.");
}
...
void ProviderBridgeEpFactory::ReleaseEp(OrtEpApi::OrtEp* /*ep*/) {
  // we never create an OrtEp so we should never be trying to release one
  ORT_THROW("No ReleaseEp call is required for ProviderBridgeEpFactory.");
}
}  // namespace onnxruntime
